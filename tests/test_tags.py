...
async def test_words_by_tag_resolves_via_tag_key(engine):
    lex = await _seed_browse(engine)
    upper = await lex.words_by_tag("Business")
    lower = await lex.words_by_tag("business")
